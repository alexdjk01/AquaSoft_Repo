...
export class HotelsService {
  ...
  async findAllOffersByGroupId(groupId: number): Promise<HotelOffers[]> {
    // 1. retrieve all hotels in  group
    const hotelsInGroup = await this.hotelModel.findAll({
      where: { GroupID: groupId },
      attributes: ['HotelID'], // Get only the HotelID field
    });
  
    console.log('Hotels in Group:', hotelsInGroup);
  
    // 2. get the HotelIDs
    const hotelIds = hotelsInGroup.map((hotel) => hotel.dataValues.HotelID);
  
    console.log('Extracted Hotel IDs:', hotelIds);
  
    // No hotels => return an empty array
    if (hotelIds.length === 0) {
      return [];
    }
  
    // 3. retrieve all offers where HotelID matches the IDs
    return this.hotelOfferModel.findAll({
      where: {
        HotelID: hotelIds,
      },
