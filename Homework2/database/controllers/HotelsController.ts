...
const hotelService:HotelsService = new HotelsService();
...
export class HotelsController{
    ...
    async getAllHotels(req:Request, res:Response)   
    {
        ...
        res.json(hotels); res.status(200);
    }
    ...
    async getHotelByName(req: Request, res: Response) 
    {
    ...
    async updateHotel(req: Request, res: Response) 
    {
        ...
        if (rowsUpdated > 0) res.json({ message: "Hotel updated successfully" });
    ...
    async deleteHotel(req: Request, res: Response) 
    {
        ...
        if (rowsDeleted > 0) res.json({ message: "Hotel deleted successfully" });
        else res.status(404).json({ message: "Hotel not found" });
    }
